...
$stmt = $pdo->prepare("SELECT * FROM airline WHERE airlineID='".$_GET['edit']."' ;");
$stmt->execute();// $Resultt= mysqli_query( $Connect, $SQL);
// set the resulting array to asscoiative
$result = $stmt->setFetchMode(PDO::FETCH_ASSOC);/////////what is this ?????????
foreach($stmt->fetchAll() as $row) {//while($row= mysqli_fetch_assoc($Resultt)){
 $name= $row["name"];  
...
if(isset($_POST['update'])){
 ...
 if(!empty($file)) {
    try{
    $target_file = $target_dir . basename($file["pic"]["name"]);

    // Check file size
    if($file["pic"]["size"] > 50000000) {
      exit("Image size is too large.");
    }else{
      move_uploaded_file($file["pic"]["tmp_name"], $target_file);
 ...
 // echo 'UPDATE airline SET name="'.$_POST["airlinename"].'" , description="'.$_POST['des'].'" , img="'.$airlineImg.'" WHERE airlineID="'.$ID.'"'; exit;
  $stmt=$pdo->prepare('UPDATE airline SET name="'.$_POST["airlinename"].'" , description="'.$_POST['des'].'" , img="'.$airlineImg.'" WHERE airlineID="'.$ID.'"'); 
$stmt->execute();// $Resultt= mysqli_query( $Connect, $SQL);
...
$airlineImg = null;
 ...
 if(!empty($file)) {
  ...
  $stmt=$pdo->prepare("UPDATE airline SET name = '".$_POST['airlinename']."', description= '".$_POST['des']."' ,img='".$airlineImg."'
WHERE airlineID ='".$ID."' ;";
 * $stmt->execute();// $Resultt= mysqli_query( $Connect, $SQL);
*/

}//for isset the big one
...
<script>
  function checkText() {
    ...
    if (text == "") {
      ...
      var answer = window.confirm("Are you sure you want to update this airline?");
            if (answer) {
      
                alert(document.getElementById("airlinename").value + " have been updated successfully!");
    ...
    <div class="container">
      ...
      <div class="form">
        ...
        <div class="sign-up" id="sign-up-info">
          ...
          <form id="sign-up-form"  enctype="multipart/form-data" action="" method="POST">
               <div class="picture-container">
            <div class="picture">
              <img src="" alt="" id="person-img" class="picture-src" title="">
              <input type="file" name="pic" id="wizard-picture" class="">
            </div>
            <h6 class="ch-Picture">Choose The logo</h6>

          </div>

              <input type="text" id="airlinename" name="airlinename" value="<?php echo $name; ?>" placeholder=" updated Airline Name" />
            <input type="text" id="airliDES" name="des"  value="<?php echo $Description;?>"placeholder="updated Description" />
             
            
            
   
            <input type="submit" class="control-button up" name="update" value="Update" role="button"
              onClick="checkText();" id="submit-button">
